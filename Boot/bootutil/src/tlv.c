                        ...
                        const struct flash_area *fap, uint16_t type, bool prot)
{
    ...
    if (info.it_magic == IMAGE_TLV_PROT_INFO_MAGIC) {
        if (info.it_tlv_tot < sizeof(info) ||
            hdr->ih_protect_tlv_size != info.it_tlv_tot) {
            return -1;
        }
        if (!boot_u32_safe_add(&tlv_info_off, off_, info.it_tlv_tot)) {
            return -1;
        }

        if (LOAD_IMAGE_DATA(hdr, fap, tlv_info_off, &info, sizeof(info))) {
            return -1;
        }
    ...
    it->type = type;
    it->prot = prot;
    if (!boot_u32_safe_add(&it->prot_end, off_, it->hdr->ih_protect_tlv_size)) {
        return -1;
    }
    ...
    if (!boot_u32_safe_add(&it->tlv_off, off_, sizeof(info))) {
        return -1;
    }
    return 0;
}
                       ...
                       uint16_t *type)
{
    ...
    while (it->tlv_off < it->tlv_end) {
        ...
        if (!boot_u32_safe_add(&data_off, it->tlv_off, sizeof(tlv)) ||
            data_off > it->tlv_end) {
            return -1;
        }
        ...
        if (!boot_u32_safe_add(&next_off, data_off, tlv.it_len) ||
            next_off > it->tlv_end) {
            return -1;
        }

        if (it->hdr->ih_protect_tlv_size > 0 &&
            it->tlv_off < it->prot_end && next_off > it->prot_end) {
            return -1;
        }
