...
## G6 P0 D10 Anti-Rollback

Boot owns final anti-rollback. The application image carries MCUboot's native
`IMAGE_TLV_SEC_CNT` (`0x50`) in the protected TLV area. The value is generated by
`imgtool sign --security-counter` and is covered by the existing SHA-256 and
ECDSA validation path.

The common bootutil execution path is intentionally kept aligned with MCUboot:

- `bootutil_img_validate()` validates `IMAGE_TLV_SEC_CNT` against
  `boot_nv_security_counter_get()`.
- `boot_update_hw_rollback_protection()` runs after selected-slot validation.
- `boot_update_security_counter()` reads the image counter through
  `bootutil_get_img_security_cnt()` and advances it with
  `boot_nv_security_counter_update()`.

MCUboot does not define fixed flash partitions for the NV security counter. This
STM32U5 port implements MCUboot's `boot_nv_security_counter_*()` interface using
two redundant Boot-owned flash pages:

- `FLASH_AREA_BOOT_SECURITY_STATE_0`: `0x081F6000..0x081F7FFF`
- `FLASH_AREA_BOOT_SECURITY_STATE_1`: `0x081F8000..0x081F9FFF`

The ECU journal pages remain at `0x081FA000..0x081FDFFF`. The Boot user area
active-slot record remains at `0x081FE000..0x081FFFFF`.

Boot rejects an otherwise valid image when its protected security counter is
below the stored counter. Gateway and ECU downgrade checks are defense-in-depth
and evidence quality only.

The direct-XIP revert trailer remains the activation, confirmation, and rollback
truth source. Boot validates the security counter before boot, but it advances
the stored counter only after the selected image is confirmed with
`image_ok == BOOT_FLAG_SET`. Boot does not write `image_ok` during selection. The
newly booted runtime remains the only confirmation writer after startup
self-check.
